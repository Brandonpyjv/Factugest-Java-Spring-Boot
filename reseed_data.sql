-- ============================================================
-- Re-seed PostgreSQL data from MariaDB backup (factugest.sql)
-- Run this against: jdbc:postgresql://localhost:5432/factugest
-- ============================================================

-- Step 1: Truncate all tables (CASCADE handles FK dependencies)
TRUNCATE TABLE
    detalle_factura,
    factura_descuento,
    factura_impuesto,
...
-- Step 2: Insert independent tables first
...
SELECT setval('impuestos_cod_impuesto_seq', 9);
...
SELECT setval('usuarios_cod_usuario_seq', 6);
...
SELECT setval('pagos_factura_cod_pago_factura_seq', 8);
...
SELECT setval('metodos_pago_cod_pago_seq', 7);
...
SELECT setval('descuentos_cod_descuento_seq', 665659);
...
SELECT setval('customers_customer_id_seq', 13);
...
SELECT setval('empresas_cod_empresa_seq', 7);

-- Step 3: productos (depends on impuestos)
INSERT INTO productos (cod_producto, sku, nombre, descripcion, precio_unitario, stock, stock_minimo, cod_impuesto, unidad_medida, codigo_barras, activo) VALUES
(1,  'TEC-001', 'Logitech Wireless Mouse',        'Ergonomic black mouse, AA battery included',            45000.00, 50,  10, 1, 'C62', NULL, 1),
(2,  'TEC-002', 'RGB Mechanical Keyboard',         'Gaming keyboard with blue switches, backlit',          180000.00, 20,  5, 1, 'C62', NULL, 1),
(3,  'TEC-003', 'Samsung 24" Monitor',             'LED IPS 75Hz display, HDMI/VGA',                      650000.00,  2,  3, 1, 'C62', NULL, 1),
(4,  'ACC-001', 'Laptop Backpack',                 'Waterproof backpack for laptops up to 15.6"',           95000.00, 30,  5, 1, 'C62', NULL, 1),
(5,  'SER-001', 'Preventive Maintenance',          'PC cleaning and optimization technical service',        80000.00,999,  0, 1, 'WSD', NULL, 1),
(6,  'TEC-004', '480GB SSD Solid State Drive',     'Kingston SATA III solid state drive',                 120000.00, 40,  8, 1, 'C62', NULL, 1),
(7,  'TEC-005', '2 Meter HDMI Cable',              'Reinforced 4K high-speed cable',                       15000.00, 15, 20, 1, 'C62', NULL, 1),
(8,  'LIC-001', '1 Year Antivirus License',        'Digital activation code sent via email',               55000.00,999,  0, 1, 'WSD', NULL, 1),
(9,  'TEC-006', '8GB DDR4 RAM Memory',             'Laptop memory module 2666MHz',                        110000.00, 25,  5, 1, 'C62', NULL, 1),
(10, 'PAP-001', 'Letter Size Bond Paper Ream',     'White paper box 75g x 500 sheets',                     18500.00,  0,  0, 1, 'C62', NULL, 0);

SELECT setval('productos_cod_producto_seq', 11);

-- Step 4: facturas (depends on customers, usuarios, empresas, pagos_factura, metodos_pago)
...
SELECT setval('facturas_cod_factura_seq', 15);

-- logs (depends on usuarios)
...
SELECT setval('logs_id_log_seq', 6);

-- detalle_factura (depends on facturas, productos)
...
(1, 10, 1, 2, 45000,  90000,  0, 0, 0,  0),
(2, 12, 9, 1, 110000, 110000, 0, 0, 19, 20900),
(3, 13, 9, 1, 110000, 110000, 0, 0, 19, 20900),
(4, 13, 3, 1, 650000, 650000, 0, 0, 19, 123500),
(5, 14, 4, 1, 95000,  95000,  0, 0, 19, 18050);

SELECT setval('detalle_factura_cod_destalle_seq', 6);

-- producto_descuento (depends on productos, descuentos)
